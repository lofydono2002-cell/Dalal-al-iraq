...
import { Building2, Users, Eye, Trash2, CheckCircle, XCircle, Handshake, Plus, MapPin, Phone, Loader2, Pin, Flag } from "lucide-react";
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"listings" | "offices" | "reports">("listings");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      {/* Tabs */}
      <div className="flex gap-2 mb-5 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit">
        <button onClick={() => setTab("listings")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${tab === "listings" ? "bg-white dark:bg-gray-700 text-orange-500 shadow-sm" : "text-gray-500 dark:text-gray-400"}`}>
          إدارة الإعلانات
        </button>
        <button onClick={() => setTab("offices")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${tab === "offices" ? "bg-white dark:bg-gray-700 text-orange-500 shadow-sm" : "text-gray-500 dark:text-gray-400"}`}>
          المكاتب الدلالية
        </button>
        <button onClick={() => setTab("reports")}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 ${tab === "reports" ? "bg-white dark:bg-gray-700 text-orange-500 shadow-sm" : "text-gray-500 dark:text-gray-400"}`}>
          البلاغات
          {openCount > 0 && (
              ...
              offices.map((office) => (
                <div key={office.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-4 flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <p className="font-bold text-gray-800 dark:text-gray-100 text-sm mb-1">{office.name}</p>
